...
anova(model2,model4)
#### -> significantly different! -> Include both intercepts and slopes

#### model 2c -- With varying intercepts and slopes for only item
model2c <- lmer(norm_exp ~ cond_c + (1 + cond_c|target_no), data = normalized_data_pq1)
summary(model2c)

#### compare model 2 and 2c
anova(model2,model2c)
#### -> significantly different! -> Include subject as random effect

#### model 2d -- With varying intercepts and slopes for only subject
model2d <- lmer(norm_exp ~ cond_c + (1 + cond_c|id), data = normalized_data_pq1)
summary(model2d)

#### compare model 2 and 2d
anova(model2,model2d)
#### -> not significantly different! -> Don't include item as random effect???

#### model 2e -- With varying intercepts for only subject
model2e <- lmer(norm_exp ~ cond_c + (1|id), data = normalized_data_pq1)
summary(model2e)

#### compare model 4 and 2e
anova(model4,model2e)
#### -> not significantly different! -> Don't include item as random effect???

#### compare model 2d and 2e
anova(model2d,model2e)
#### -> significantly different! ->Include subject as random effect with slopes and intercept, but not item
