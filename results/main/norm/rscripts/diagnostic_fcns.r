...
overdisp.test<-function(x){
  ...
  sum.dp=sum(pr^2)
  if(class(x)[[1]]=="lmerMod" | class(x)[[1]]=="glmerMod"){
    xdf=length(residuals(x))-length(fixef(x))-nrow(as.data.frame(summary(x)$varcor))
  }else{
    xdf=length(residuals(x))-length(x$coefficients)
  }
  xdf=xdf-length(summary(x)$theta)
  if(class(x)[[1]]=="glmerMod"){
		xx=as.character(x@call)[4]
		if(grepl(x=xx, pattern="negative.binomial")){xdf=xdf-1}
	}
  return(data.frame(chisq=sum.dp, df=xdf, P=1-pchisq(sum.dp, xdf), dispersion.parameter=sum.dp/xdf))
}

fe.re.tab<-function(fe.model, re, other.vars=NULL, data, treat.covs.as.factors=c(F, T, NA)){
	#function helping in determinining which random slopes are needed
	#last updated: 2017, Dec 28
	#latest updates:
		...
		#output gives information about which fixed effects are covariates and factors aso for interactions
			#and also how covariattes were treated
	#input/arguments:
	...
	if(nrow(to.do)>0){
		...
		to.add=lapply(1:nrow(to.do), function(xrow){#treat combinations of fixed and random effect by lapply-ing over the rows of to.do
			...
			all.tabs=lapply(1:length(iterms), function(yrow){#for each term
				...
				#otherwise, determine per level of the random effect the number of levels of the fixed effect with at least two observations
				apply(res.detailed[[paste(c(iterms[yrow], to.do[xrow, "re"]), collapse="_within_")]]>ifelse(imodes[yrow]=="factor", 1, 0), 1, sum)
			})
